...
int CUDPSocket::read(char* buffer, size_t length, sockaddr_storage& address, unsigned int &address_length)
{
	assert(buffer != NULL);
	assert(length > 0);
	...
	ssize_t len = ::recvfrom(pfd[index].fd, buffer, length, 0, (sockaddr *)&address, &size);
	if (len <= 0) {
...
bool CUDPSocket::write(const char* buffer, size_t length, const sockaddr_storage& address, unsigned int address_length)
{
	assert(buffer != NULL);
	assert(length > 0);
	...
	for (int i = 0; i < UDP_SOCKET_MAX; i++) {
		...
		ssize_t ret = ::sendto(m_fd[i], buffer, length, 0, (sockaddr *)&address, address_length);
		if (ret < 0) {
