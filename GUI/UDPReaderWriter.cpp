/*
 *   Copyright (C) 2006-2012,2021 by Jonathan Naylor G4KLX
 *
...
int CUDPReaderWriter::read(char* buffer, unsigned int length)
{
	...
	ssize_t len = ::recvfrom(m_fd, buffer, length, 0, (sockaddr *)&addr, &size);
	if (len < 0) {
#if defined(__WINDOWS__)
	...
	if (m_remAddr.sin_addr.s_addr != addr.sin_addr.s_addr || m_remAddr.sin_port != addr.sin_port) {
		...
		CUtils::dump(wxT("Data"), (unsigned char *)buffer, len);
		return 0;
	}
...
bool CUDPReaderWriter::write(const char* buffer, unsigned int length)
{
	ssize_t ret = ::sendto(m_fd, buffer, length, 0, (sockaddr *)&m_remAddr, sizeof(sockaddr_in));
	if (ret < 0) {
#if defined(__WINDOWS__)
